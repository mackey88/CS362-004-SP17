...
// Play the adventurer card, taking in parameters from original switch statement
// BUG: <= 2 lets the player draw 3 cards
int adventurerCard(struct gameState *state, int currentPlayer, int cardDrawn, int drawntreasure, int temphand[], int z){
...
// Play the feast card, taking in parameters from original switch statement
// BUG: i < state->handCount[currentPlayer] does not correctly copy over hand to tempHand
...
// Play the feast card, taking in parameters from original switch statement
// BUG: state->hand[currentPlayer][choice1]) + 3 allows player to gain a card costing up to +3 rather than +2
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurerCard(state, currentPlayer, cardDrawn, drawntreasure, temphand, z);
